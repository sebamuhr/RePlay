...
- ☁️ **Big local pool** — up to **5,000** videos are cached on-device, so your feed and searches are instant and free (no API call). The feed paints in batches with a **Show more** button so it stays smooth.
...
- 🖥 **Horizontal** and 📱 **Vertical** split your feed by orientation. The cutoff is duration-based (configurable in ⚙) but you can override any video with the phone button above.
- 📺 **Channels** sub-tab (on Search) lets you search channels right from the feed.

### 🌙 Blackout mode (small screens)
- A **🌙** button appears inside the player. Start a video, tap 🌙, and the screen goes **fully black** while audio keeps playing.
...
- Dim **⏮ ⏯ ⏭** controls in the center let you go previous / play-pause / next without leaving the dark; a small **✕** in the corner closes it.
...
- 👍 **Follow / 🚫 Hide channels** while watching. The LED dot lights 🔴 when a followed channel has new uploads (cheap: ~1 quota unit per channel). Import your YouTube subscriptions from a Takeout CSV.
- ☁️ **Sync across devices** via a private GitHub Gist (optional). Lists, pool, followed channels, orientation overrides and Music all merge across devices (with tombstones, so deletions stick).
...
- The YouTube API has **no "is this a Short / orientation" flag**, so RePlay uses **duration** as a proxy (configurable cutoff, default **< 60s = Vertical**). A genuinely short normal clip may land on the Vertical side — fix it with the 📱 button.
